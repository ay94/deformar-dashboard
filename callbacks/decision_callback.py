...
import json
import dash
from utils.layout_managers import get_input_trigger, process_json_data, process_selection
...
def register_callbacks(app, variants_data):
    ...
    def generate_matrix(n_clicks, correlation_method, variant):
        ...
        if n_clicks is not None and n_clicks > 0:
            ...
            matrix_style = {'width': '100%', 'height': '500px', 'display': 'inline-block'}
    ...
    def generate_decision_plot(n_clicks, measureSelection, variant, decision_type, color_columns):
    ...
    def generate_measure_plot(n_clicks, clickData, decisionSelection, variant, color_columns):
        if n_clicks is None or n_clicks == 0:
        ...
        symbol_column = color_columns[1] if color_columns and len(color_columns) > 1 else None
    ...
    def store_decision_selection(selectedData, _):
        ...
        if trigger_id == 'clear_decision':
            ...
            if selectedData and 'points' in selectedData:
                extracted_data = [
                    {'Global Id': point['customdata'][0]} for point in selectedData['points']
    ...
    def generate_selection_output(measureSelection, decisionSelection, variant):
        ...
        if decision_selection_ids:  # Use decision selection if available
            ...
            return html.Div("No data selected from measure or decision plots.", className='prompt-message')
    ...
    def view_training_impact(variant):
        attention_weights, attention_matrices = tab_manager.generate_training_impact(variant)
        ...
        if attention_weights is  None and attention_matrices is None:
            return html.Div(
                ...
                "No Attention Matrices Available",
                className='prompt-message'
            )
