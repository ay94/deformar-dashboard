...
from utils.tab_managers import BaseAnalysis
...
from utils.config_managers import MatrixConfig, DecisionScatterConfig, ScatterConfig, ColorMap
from utils.tab_managers import BasePlotting, create_violin_plot, create_histogram_plot, create_scatter_plot, create_bar_chart, create_correlation_matrix_plot, create_similarity_matrix_plot, create_scatter_width_plot, create_scatter_plot_with_color

def create_confusion_table(errors, true_col, pred_col):
    confusion = errors.pivot_table(index=true_col, columns=pred_col, aggfunc='size', fill_value=0)
    return confusion
...
class TrainScatter(BasePlotting):
    def generate_plot(self, selected_df,):
        ...
        return create_scatter_plot_with_color(selected_df, columns.X_COLUMN.value, columns.Y_COLUMN.value, columns.TRUE_LABELS.value, None, scatter_config)
        
class DecisionScatter(BasePlotting):
    def generate_plot(self, selected_df, x_column, y_column, color_column, symbol_column):
        """
        Generate the token length distribution plot.
        """
        ...
        return create_scatter_plot_with_color(selected_df, x_column, y_column, color_column, symbol_column, scatter_config)
...
class SelectionTagProportion(BaseAnalysis):
    ...
    def generate_plot(self, df, columns):
        ...
        confusion_table = create_confusion_table(df, columns.TRUE_LABELS.value, columns.PRED_LABELS.value)
        color_map = ColorMap()
